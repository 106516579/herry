...
jQuery.fn.timelinr = function(options){
	...
	settings = jQuery.extend({
		...
		issuesSpeed: 				700,				// value: integer between 100 and 1000 (recommended), default to 200 (fast)
		...
		autoPlayPause: 				1000				// value: integer (1000 = 1 seg), default to 2000 (2segs)
	...
	$(function(){
		...
		$(settings.datesDiv+' a').click(function(event){
			...
			$(settings.issuesDiv+' li').animate({queue:false, duration:settings.issuesSpeed}).removeClass(settings.issuesSelectedClass).eq(currentIndex).addClass(settings.issuesSelectedClass).fadeTo(settings.issuesTransparencySpeed,1);
			...
			if(settings.orientation == 'horizontal') {
				...
				if(currentPositionIssues <= -(widthIssue*howManyIssues-(widthIssue))) {
					...
					if (!$(settings.issuesDiv).is(':animated')) {
						...
						$(settings.issuesDiv+' li').animate({queue:false, duration:settings.issuesSpeed});
